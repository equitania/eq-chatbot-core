...
class IonosProvider(BaseLLMProvider):
    """
    ...
    def __init__(
        ...
        """
        Initialize the IONOS provider.

        Args:
            api_key: IONOS API token sent as ``Authorization: Bearer <api_key>``.
            base_url: OpenAI-compatible endpoint. Defaults to the official IONOS
        ...
        # SSRF guard: IONOS is a fixed public cloud endpoint, so private ranges
    ...
    def stream_completion(
        ...
        **kwargs,
        ...
        """Stream a chat completion response from the IONOS AI Model Hub."""
        model = model or self.default_model
    ...
    def list_models(self) -> list[dict[str, Any]]:
        """
        List models advertised by the IONOS AI Model Hub.
    ...
    def __enter__(self) -> "IonosProvider":
        return self
