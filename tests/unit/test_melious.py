"""
Unit tests for the Melious.ai provider (OpenAI-compatible, sovereign EU-hosted).
...
def mock_chat_response():
    ...
    response = MagicMock()
    response.model = "minimax-428b-m3"
    response.choices = [MagicMock()]
    response.choices[0].message.content = "Hallo aus der souveränen EU-Cloud."
    response.choices[0].message.tool_calls = None
    response.choices[0].finish_reason = "stop"
    response.usage = MagicMock()
    response.usage.prompt_tokens = 11
    response.usage.completion_tokens = 7
    response.model_dump.return_value = {"model": "minimax-428b-m3"}
    return response
...
def mock_models_list():
    """Mock /v1/models response — Melious serves 60+ open-weight models."""
    models = MagicMock()
    models.data = [
        MagicMock(id="minimax-428b-m3", created=1700000000, owned_by="minimax"),
        MagicMock(id="deepseek-v3.1", created=1700000000, owned_by="deepseek"),
...
def _use_melious_openai_mock():
    """Install our openai mock for each test, then restore the prior entry.

    Other provider test modules (e.g. test_openai.py, test_ionos.py) also
...
class TestMeliousProviderInit:
    ...
    def test_base_url_optional(self):
        # Like IONOS, Melious does not require an explicit base_url.
        provider = MeliousProvider(api_key="test-key")
        assert provider.base_url == DEFAULT_BASE_URL

    def test_base_url_override(self):
        provider = MeliousProvider(api_key="test-key", base_url=TEST_BASE_URL)
        assert provider.base_url == TEST_BASE_URL

    def test_ssrf_metadata_blocked(self):
        with pytest.raises(ValueError):
            MeliousProvider(api_key="test-key", base_url="http://169.254.169.254/v1")

    def test_private_range_blocked(self):
        # Melious is a fixed public endpoint; private ranges are rejected.
        with pytest.raises(ValueError):
            MeliousProvider(api_key="test-key", base_url="http://10.0.0.5/v1")

    def test_non_http_scheme_blocked(self):
        with pytest.raises(ValueError):
            MeliousProvider(api_key="test-key", base_url="file:///etc/passwd")
...
@pytest.mark.unit
class TestMeliousProviderProperties:
...
@pytest.mark.unit
class TestMeliousChatCompletion:
    def test_simple_completion(self, mock_chat_response):
        ...
        assert response.content == "Hallo aus der souveränen EU-Cloud."
        assert response.model == "minimax-428b-m3"
    ...
    def test_explicit_model_and_max_tokens(self, mock_chat_response):
        ...
        assert kwargs["model"] == "deepseek-v3.1"
    ...
    def test_temperature_clamped(self, mock_chat_response):
        ...
        # Open-weight models support 0.0-2.0; a normal value passes through.
    ...
    def test_melious_specific_kwargs_passed_through(self, mock_chat_response):
        # Melious-specific extras (preset / request_id) ride through **kwargs.
        mock_client = MagicMock()
        mock_client.chat.completions.create.return_value = mock_chat_response
        provider = _make_provider_with_client(mock_client)
        ...
        assert mock_client.chat.completions.create.call_args.kwargs["preset"] == "reasoning"
...
@pytest.mark.unit
class TestMeliousStreamCompletion:
...
@pytest.mark.unit
class TestMeliousErrorHandling:
    def _provider_raising(self, exc: Exception) -> MeliousProvider:
        mock_client = MagicMock()
    ...
    def test_error_scrubs_secret(self):
        provider = self._provider_raising(Exception("500 error for key sk-mel-leakedsecret12345"))
        with pytest.raises(ProviderError) as exc_info:
            provider.chat_completion(messages=[{"role": "user", "content": "Hi"}])
        assert "sk-mel-leakedsecret12345" not in str(exc_info.value)
...
@pytest.mark.unit
class TestMeliousContextManager:
    ...
    def test_context_manager(self):
        mock_client = MagicMock()
        mock_openai_module.OpenAI = MagicMock(return_value=mock_client)
        with MeliousProvider(api_key="x") as provider:
            provider._client = None
            _ = provider.client
        mock_client.close.assert_called_once()
